#!/usr/bin/env python3
"""
T0856 - Sensor Spoofing: Heading Injection 90°
WebSocket üzerinden inject_heading komutu - host'tan çalıştırılır
Kullanım: python3 attack_t0856_heading090.py
NOT: Önce heading 10 dereceye reset edilmeli
"""
...
async def main():
    import aiohttp
    print("[T0856] Heading Injection 90 derece basliyor...")
    collector = run_collector("t0856_heading_090", DURATION)
    ...
    async with aiohttp.ClientSession() as session:
        async with session.ws_connect(CC_WS_URL + "/command",
            ...
            await ws.send_json({"jsonrpc":"2.0","method":"attack_start",
                                "params":["inject_heading", 90, 10],"id":1})
            resp = await asyncio.wait_for(ws.receive_json(), timeout=5)
            print(f"[WS] {resp}")

            if "error" in resp:
                print("[ERROR] Saldiri baslatılamadı — önceki inject hala aktif olabilir")
                collector.kill()
                return
